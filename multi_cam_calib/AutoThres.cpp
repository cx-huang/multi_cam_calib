...
void AutoThres(Mat src, Mat &dst)
{
	Mat hist;
	/* 计算直方图 */
	int bins=256;	//设定像素的取值范围，因为传入的图像是经过split()后得到的单通道图像，所以像素取值范围为0-255
	int histSize[] = {bins};
	float range[] = {0,256};
	...
	threshold(src,dst,T_new+15,255,CV_THRESH_BINARY);	//阈值是T_new+15，最大值是255
													//CV_THRESH_BINARY：>(T_new+15)的像素赋值为255，否则赋值为0
}
